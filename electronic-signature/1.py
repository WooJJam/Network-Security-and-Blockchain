...
def sign(M, d):
    ...
    P = double_and_add(k,e1)
    S1 = P[0] % q
    S2 = (e + d * S1)*find_inverse(k,q)
    return S1, S2

def verify(M, S1, S2, e2):
    A = int(hashlib.sha256(M.encode()).hexdigest(), 16) * find_inverse(S2, q)
    B = S1 * find_inverse(S2, q)
    T = add(double_and_add(A, e1), double_and_add(B, e2))
    return (T[0] % q) == (S1 % q) 
    
if __name__ == "__main__":
    d = get_private_key() # 2주차 과제에서 작성한 함수
    e2 = double_and_add(d, e1) # 2주차 과제에서 작성한 함수
